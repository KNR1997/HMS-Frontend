...
export default function OrderDetailsPage() {
  ...
  const { booking, isLoading: loading, error } = useBookingQuery({
    booking_number: query.bookingNumber as string,
    language: locale!,
  });
  ...
  const {
    ...
    defaultValues: { order_status: booking?.status ?? "" },
  });

  const ChangeStatus = ({ order_status }: FormValues) => {
    updateBooking({
      id: booking?.booking_number as string,
      status: order_status?.status as string,
    });
  };

  const handleDeleteBooking = () => {
    deleteBooking({ id: booking?.id as string });
  };

  const handleDeleteRoomItem = (bookingItemId: string) => {
    openModal("DELETE_BOOKING_ITEM", { id: bookingItemId });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "price_per_night",
      align: alignLeft,
      render: (price_per_night: string) => <span>Rs. {price_per_night}</span>,
    },
    // {
    //   dataIndex: 'image',
    //   key: 'image',
    ...
    // },
    {
      ...
      align: "right",
      width: 260,
      render: (slug: string, record: BookingItem) => (
        <button
          onClick={() => handleDeleteRoomItem(record.id)}
    ...
    <>
      <Card className="relative overflow-hidden">
        <div className="mb-6 -mt-5 -ml-5 -mr-5 md:-mr-8 md:-ml-8 md:-mt-8">
          <OrderViewHeader booking={booking} wrapperClassName="px-8 py-4" />
        </div>
        ...
        <div className="flex flex-col items-center lg:flex-row">
          ...
          ].includes(booking?.status! as OrderStatus) && (
            <form
        ...
        <div className="my-5 flex items-center justify-center lg:my-10">
          <OrderStatusProgressBox
            orderStatus={booking?.status as OrderStatus}
